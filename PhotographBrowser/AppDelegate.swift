...
// MARK: - 根据image来计算ImageView的Frame -> CGRect
func calculateImageViewFrame(image : UIImage) -> CGRect {
    let imageViewW = UIScreen.mainScreen().bounds.width
    let imageViewH = imageViewW / image.size.width * image.size.height// 根据宽高比计算
    let imageViewX : CGFloat = 0
    let imageViewY = (UIScreen.mainScreen().bounds.height - imageViewH) * 0.49
